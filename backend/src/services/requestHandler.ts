...
import { validateLicenseToken } from "./licenseValidationService.ts";
...
export const handleStreamRequest = async (
  ...
  try {
    const { text: paragraphs, licenseToken, options } = await ctx.request.body.json() as { text: Paragraph[], licenseToken: string, options: any };
    ...
    console.log("[HANDLER] 3. Licença validada");
    ...
    if (!paragraphs || paragraphs.length === 0) {
      ...
      ctx.response.body = { error: "O parâmetro 'text' (array de parágrafos) é obrigatório." };
      return;
    }
    ...
    try {
          ...
          try {
            for await (const chunk of aiStream) {
              ...
              while ((newlineIndex = buffer.indexOf('\n')) !== -1) {
                ...
                if (cleanedLine && cleanedLine.startsWith('{') && cleanedLine.endsWith('}')) {
            ...
            console.log("[HANDLER] 8. Stream da IA finalizado e enviado para o cliente.");
          } catch (streamError) {
            logger.error({ err: streamError }, `Erro durante o streaming da resposta da IA para /api/v1/${actionName}:`);
      ...
      logger.error({ err: innerError }, `Erro na chamada de IA para /api/v1/${actionName}:`);
      const errorMessage = innerError instanceof Error ? innerError.message : String(innerError);
      track("error", { type: "api_error", message: errorMessage, route: `/api/v1/${actionName}` });
      
      if (ctx.response.writable) {
          ctx.response.status = 500;
          ctx.response.body = { error: "Erro interno ao processar a solicitação de IA." };
      }
    }
  } catch (outerError) {
    console.error("[HANDLER] Erro catastrófico em handleStreamRequest:", outerError);
    ...
    if (ctx.response.writable) {
      ctx.response.status = 500;
      ctx.response.body = { error: "Um erro crítico e inesperado ocorreu no servidor." };
    }
  }
};
