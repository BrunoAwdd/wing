...
export const validateLicenseToken = async (
  ...
  // --- BYPASS PARA TESTES ---
  if (token === "wing_test_dummy_token") {
    console.log("[LICENSE-VALIDATION] Usando token dummy para testes.");
    return {
      isValid: true,
      entitlement: "Paid",
  ...
  if (apiKeyError || !apiKeyData) {
    console.error(
      "[LICENSE-VALIDATION] Chave de API não encontrada ou erro:",
      apiKeyError?.message
    );
    return { isValid: false, entitlement: "Invalid", userId: null };
  }
  ...
  const dbHash = new Uint8Array(
    dbHashString.match(/.{1,2}/g)!.map((byte) => parseInt(byte, 16))
  );
  ...
  if (apiKeyData.status !== "active") {
    console.warn(
      `[LICENSE-VALIDATION] Chave de API ${apiKeyData.id} não está ativa.`
    );
    return {
      isValid: false,
  ...
  supabase
    ...
    .then(({ error }) => {
      if (error)
        console.error(
          "[USAGE-UPDATE] Erro ao atualizar last_used_at:",
          error.message
        );
    });
  ...
  console.log(
    `[LICENSE-VALIDATION] Token validado com sucesso para o usuário ${apiKeyData.user_id}.`
  );

  // Mapeia os scopes para um único entitlement. Simplificação.
  const entitlement = (apiKeyData.scopes as string[]).includes("rewrite")
    ? "Paid"
    : "Free";
